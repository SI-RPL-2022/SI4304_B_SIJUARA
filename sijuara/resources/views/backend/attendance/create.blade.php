...
@section('content')
    <div class="create">
        <div class="flex items-center justify-between mb-6">
            <div>
                <h2 class="text-gray-700 uppercase font-bold">DAFTAR KEHADIRAN SISWA KELAS {{ $class->class_name }}</h2>
            </div>
        ...
        <div class="w-full mt-8 bg-white rounded">
            <div class="flex items-center justify-between px-6 py-6 pb-0">
                ...
                <h3 class="text-gray-700 uppercase font-bold"> {{ date('d-m-Y') }}</h3>
            </div>

            <div class="w-full px-6 py-6">
                <div class="flex items-center bg-gray-200 rounded-tl rounded-tr">
                    <div class="w-4/12 text-center text-gray-600 py-2 px-4 font-semibold">Nama Siswa</div>
                    <div class="w-3/12 text-center text-gray-600 py-2 px-4 font-semibold">Roll</div>
                    <div class="w-5/12 text-right text-gray-600 py-2 px-4 font-semibold">Status</div>
                </div>
                <form action="{{ route('teacher.attendance.store') }}" method="POST">
                    @foreach ($class->students as $student)
                        <div class="flex items-center justify-between border border-gray-200">
                            @csrf
                            <div class="w-4/12 text-sm text-center text-gray-600 py-2 px-4 font-semibold">{{ $student->user->name }}</div>
